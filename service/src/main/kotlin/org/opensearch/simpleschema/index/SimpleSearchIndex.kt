...
internal object SimpleSearchIndex : ClusterStateListener {
    ...
    /**
     * once cluster state changed reflecting nodes have joined the cluster - default index can be created
     */
    override fun clusterChanged(event: ClusterChangedEvent?) {
        // validate cluster is ready and only then create default index
        if (event != null
            && !event.state().nodes().nodes.isEmpty)
                if(!isIndexExists(INDEX_NAME)) indexCreateCall()
        }
}
