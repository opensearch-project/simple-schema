...
import org.opensearch.simpleschema.model.RestTag.DELETE_RESPONSE_LIST_TAG
 ...
 */
internal class DeleteSimpleSchemaObjectResponse : BaseResponse {
    val objectIdToStatus: Map<String, RestStatus>

    companion object {
        private val log by logger(DeleteSimpleSchemaObjectResponse::class.java)

        /**
         * reader to create instance of class from writable.
         */
        val reader = Writeable.Reader { DeleteSimpleSchemaObjectResponse(it) }
        ...
        fun parse(parser: XContentParser): DeleteSimpleSchemaObjectResponse {
            ...
            while (parser.nextToken() != XContentParser.Token.END_OBJECT) {
                ...
                when (fieldName) {
                    DELETE_RESPONSE_LIST_TAG -> objectIdToStatus = convertMapStrings(parser.mapStrings())
                        ...
                        parser.skipChildren()
                        log.info("Unexpected field: $fieldName, while parsing DeleteObjectResponse")
                    }
                }
            }
            objectIdToStatus ?: throw IllegalArgumentException("$DELETE_RESPONSE_LIST_TAG field absent")
            return DeleteSimpleSchemaObjectResponse(objectIdToStatus)
        }
